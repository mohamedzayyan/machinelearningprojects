...
class myEfigi:
    def __init__(self,train_dl, val_dl, unseen_dl, model, optimizer, scheduler, criterion, savePath='./models/efigi', device='cuda', BATCH_SIZE=64):
        self.device = device
    ...
    def change_model(self):
        self.model.fc[3] = nn.Linear(512, 9)
        del self.model.fc[4]
        return None
    ...
    def unseen_phase(self):
        self.model.eval()
        losses = []
        preds = torch.empty(0, 9).to(self.device)
        acts = torch.empty(0, 9).to(self.device)
    ...
    def train(self, n_epochs=5):
        ...
        for epoch in range(n_epochs):
            ...
            if early_stopping.early_stop:
                ep = epoch - 10
                self.model.load_state_dict(torch.load('./efigiCheckpoints/efigicheckpoint{}.pt'.format(ep)))
                print("Early stopping")
                break
        
        pickle.dump(train_losses,open('./losses/efigi_train','wb'))
        pickle.dump(val_losses,open('./losses/efigi_val','wb'))
    ...
    def show_cf(self, f1_='weighted', cf_type='numbers'):
        ...
        for i in range(orig_df.shape[0]):
            orig_df.loc[i,'cat'] = np.argmax(orig_df.iloc[i,3:].values)
        orig_df = orig_df.loc[:, ['hubb', 'cat']].drop_duplicates()
        sdf = pd.merge(df, orig_df, left_on=['Act'], right_on=['cat'], how='left')
        sdf = pd.merge(sdf, orig_df, left_on=['Predict'], right_on=['cat'], how='left')
        cf = confusion_matrix(sdf['hubb_x'].tolist(), sdf['hubb_y'].tolist())
