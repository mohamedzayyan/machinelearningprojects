...
class GalaxiesDataset_efigi(Dataset):
	...
	def __getitem__(self, idx):
		img_id = self.classes_frame.iloc[idx,1]
		img_name = os.path.join(self.root_dir, str(img_id)) + ".png"
		image = Image.open(img_name)
		labels = self.classes_frame.iloc[idx, 3:].values.astype(np.float64)
		if self.transform:
