...
class GZ2DML:
    def __init__(self,train_dl, val_dl, unseen_dl, model, optimizer, scheduler, criterion, mining_function, loss,hubble_classes=11, savePath='./models/', device='cuda', BATCH_SIZE=64):
        self.device = device
        ...
        if self.loss == 'triplet':
            ...
            self.loss_function = losses.ProxyAnchorLoss(self.hubble_classes,128, margin=0.01, alpha=5, reducer=self.reducer,
                                   weight_regularizer = self.regularizer)
        elif self.loss == 'pnca':
            self.loss_function = losses.ProxyNCALoss(self.hubble_classes, 128, softmax_scale=1, reducer = self.reducer,weight_regularizer=self.regularizer)
        elif self.loss == 'normsoftmax':
            self.loss_function = losses.NormalizedSoftmaxLoss(self.hubble_classes, 128, temperature=0.05, reducer=self.reducer,                                        weight_regularizer=self.regularizer)
        ...
        self.savePath = savePath + 'gz2{}_{}_128'.format(self.mining_function, self.loss)
